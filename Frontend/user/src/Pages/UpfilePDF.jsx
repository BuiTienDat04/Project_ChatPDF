...
export default function TranslatePDF() {
  ...
  const translateContent = async () => {
    ...
    try {
      ...
      console.log('Sending file:', file.originalFile, 'Type:', typeof file.originalFile);
      ...
      if (result.data && result.data[0] && result.data[0].translatedContent) {
        // Xóa dấu ** ở đầu mỗi câu trong translatedContent.paragraphs
        const cleanedTranslatedContent = {
          ...result.data[0].translatedContent,
          paragraphs: result.data[0].translatedContent.paragraphs.map(para =>
            para.replace(/^(\*\*)+/, '').trim()
          ),
        };
